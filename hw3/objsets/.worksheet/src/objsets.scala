...
object objsets {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(119); 
  ...
  apple.map(x=>x.toUpperCase).distinct;System.out.println("""res2: List[String] = """ + $show(res$2))}
  //TweetReader.allTweets.filter(tw=> google.exists(txt=>txt.contains(tw.text)))
}
